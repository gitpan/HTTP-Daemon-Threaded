...
Refer to the included classdocs for detailed method descriptions for all classes.
...
one of the content handler's C<getContent(), getHeader(), or putContent()> methods
...
various URIs. One exception is the special classname B<'*'>, which indicates
